 ...
 * @author chris
 */
public class QC_S extends QCOperation{

    public QC_S() {
    }
    
    public QC_S(int nqbits, int[] qubits) {
...
//        g.fillRect(timestep*xdist+leftmargin+xdist/8, ydist*this.getWires()[0]+topmargin-3*ydist/8, 3*xdist/4, 3*ydist/4);
//        Color c=g.getColor();
//        g.setColor(Color.black);
//        g.drawString("H",timestep*xdist+leftmargin+xdist/4, ydist*this.getWires()[0]+topmargin+3*ydist/16);
//        g.setColor(c);
        ...
        Color c=g.getColor();
        g.setColor(Color.black);
        g.drawString("S",timestep*xdist+leftmargin+xdist/4, ydist*this.getWires()[this.getnWires()-1]+topmargin+3*ydist/16);
        g.setColor(c);
    ...
    public String emit() {
        if(getnWires()==1){
            int n0=getWires()[0]+1;
            return "(c{"+n0+"}*c{"+n0+"}'+exp(1.0i*pi/2.0)*c{"+n0+"}'*c{"+n0+"})";
        }else if(getnWires()==2){
            int n0=getWires()[0]+1;
            int n1=getWires()[1]+1;
            return "((c{"+n1+"}*c{"+n1+"}'+exp(1.0i*pi/2.0)*c{"+n1+"}'*c{"+n1+"})*c{"+n0+"}'*c{"+n0+"}+c{"+n0+"}*c{"+n0+"}')";
        }
        return "(ERROR! S only with 1 and 2  qubits)";
    }

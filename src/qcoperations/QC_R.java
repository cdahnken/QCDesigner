 ...
 * @author chris
 */
public class QC_R extends QCOperation{
    int N=1;
    public QC_R() {
    }
    
    public QC_R(int nqbits, int[] qubits, int n) {
    ...
    public void draw(Graphics g, int timestep, int xdist, int ydist, int leftmargin, int topmargin) {
        ...
        g.drawString(""+(1<<N),timestep*xdist+leftmargin+xdist/4, ydist*this.getWires()[this.getnWires()-1]+topmargin+3*ydist/16);
        g.setColor(c);
    ...
    public String emitOctave() {
        if(getnWires()==1){
            ...
            int n0=getWires()[0]+1;
            int n1=getWires()[1]+1;
            return "((c{"+n1+"}*c{"+n1+"}'+exp(1.0i*pi/"+(1<<N)+")*c{"+n1+"}'*c{"+n1+"})*c{"+n0+"}'*c{"+n0+"}+c{"+n0+"}*c{"+n0+"}')";
        }
